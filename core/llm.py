...
class StopOnTokens(StoppingCriteria):
    ...
    def __init__(self, tokenizer, stop_sequences):
        ...
        self.stop_ids = [tokenizer.encode(stop, add_special_tokens=False) for stop in stop_sequences]
    ...
    def __call__(self, input_ids, scores, **kwargs):
        ...
        '''
        if len(input_ids) == 0 or len(input_ids[0]) == 0:
            return False
        ...
        for stop_sequence in self.stop_ids:
            ...
            if seq_len <= len(generated_tokens):
                if generated_tokens[-seq_len:] == stop_sequence:
                    return True
        return False
